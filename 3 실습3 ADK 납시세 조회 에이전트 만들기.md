# ADK(Agent Development Kit) 에이전트 만들기 
...
1. 멀티에이전트의 구조를 그림과 함께 명확히 기술해 줍니다. (위 내용 참조)
    1. 우리의 경우 A2A 에이전트가 있습니다. (매출 분석 에이전트) A2A에이전트의 위치를 정확히 알려줘야 합니다. (Agent Card)
...
```
docs/매출 데이터 분석 에이전트 개발.pdf 문서는 에이전트 개발을 위한 기획 문서입니다. 
